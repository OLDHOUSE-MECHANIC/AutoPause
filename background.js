...
// --- BEGIN: AutoPause tab-audible integration (added by patch) ---
// Map to track audible state per-tab. Prefixed with __AP_ to avoid colliding with existing identifiers.
const __AP_tabAudibleState = new Map();

function __AP_updateTabAudible(tabId, isAudible) {
  const prev = !!__AP_tabAudibleState.get(tabId);
  if (prev === !!isAudible) return;
  __AP_tabAudibleState.set(tabId, !!isAudible);

  // Notify content scripts (if present) about the authoritative tab audible state.
  // We use a new message type 'AUTO_PAUSE_TAB_AUDIBLE' to avoid interfering with existing messages.
  try {
    chrome.tabs.sendMessage(tabId, {type: 'AUTO_PAUSE_TAB_AUDIBLE', audible: !!isAudible}, () => {
      // Ignore sendMessage errors when a tab doesn't have a content script registered.
      if (chrome.runtime.lastError) {
        // console.debug('AP sendMessage error:', chrome.runtime.lastError.message);
      }
    });
  } catch (e) {
    // Defensive: background environment may not have chrome available in some tests; ignore.
  }
}

// Allow content scripts to query the current tab audible state synchronously.
chrome.runtime.onMessage.addListener((msg, sender, sendResp) => {
  if (!msg || !msg.type) return;
  if (msg.type === 'AP_QUERY_TAB_AUDIBLE') {
    const tabId = sender && sender.tab ? sender.tab.id : null;
    const value = tabId ? !!__AP_tabAudibleState.get(tabId) : false;
    sendResp({audible: value});
    return true; // keep the messaging channel open if needed
  }
});

// --- END: AutoPause tab-audible integration ---
